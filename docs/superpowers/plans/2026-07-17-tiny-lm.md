...
**Spec:** `docs/superpowers/specs/2026-07-17-tiny-lm-500m-design.md`

## Global Constraints

- No HuggingFace `transformers`; every model layer implemented from scratch in PyTorch.
...
**Files:**
- Create: `pyproject.toml`, `.gitignore`, `tinylm/__init__.py`, `tests/__init__.py`

**Interfaces:**
- Produces: importable `tinylm` package installed editable via uv; `uv run pytest -q` works.
...
- [ ] **Step 3: Create empty `tinylm/__init__.py` and `tests/__init__.py`, run `uv sync`**

Run: `uv sync && uv run python -c "import tinylm, torch; print(torch.__version__)"`
...
```python
from tinylm.config import MODEL_PRESETS, TRAIN_PRESETS, ModelConfig
...
CORPUS = [
    ...
    # enough unique pre-tokens that 255 merges are trainable (needed for vocab 512)
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor "
    "incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis "
    "nostrud exercitation ullamco laboris. " * 5,
    "Pack my box with five dozen liquor jugs; how vexingly quick daft zebras jump! "
    "Sphinx of black quartz, judge my vow. Grumpy wizards make toxic brew for the "
    "evil queen and jack. " * 5,
]
...
- [ ] **Step 3: Write `tinylm/tokenizer.py`**
...
**Files:**
- Modify: `tinylm/tokenizer.py` (append)
...
- [ ] **Step 3: Append to `tinylm/tokenizer.py`** (inside class; plus module-level helper)
...
**Files:**
- Create: `tinylm/model.py` (started here, grown in Tasks 5–6)
...
- [ ] **Step 3: Write the start of `tinylm/model.py`**
...
- [ ] **Step 3: Append to `tinylm/model.py`**
...
### Task 6: SwiGLU, Block, full TinyLM model + generate

**Files:**
- Modify: `tinylm/model.py` (append), `tinylm/__init__.py`
- Test: append to `tests/test_model.py`
...
- Produces:
  ...
  - `class TinyLM(nn.Module)`: `__init__(cfg: ModelConfig)`; modules `embed` (nn.Embedding), `blocks` (ModuleList), `final_norm`, `lm_head` (weight tied to `embed.weight`); buffers `rope_cos/rope_sin` (non-persistent).
...
def test_forward_loss_near_uniform_at_init():
    ...
    # next-token shift: unshifted targets let tied embeddings "cheat" via the
    # residual self-token dot product, inflating the init loss above ln(V)+1
...
def test_generate_respects_eos():
    ...
    eos = greedy[0, 3].item()  # first generated token, robust to greedy repetition
...
- [ ] **Step 5: Commit**: `git add -A && git commit -m "feat: full TinyLM model with tied embeddings and KV-cached generate"`
...
- Consumes: `TrainConfig` (Task 1), `TinyLM` (Task 6).
...
- Produces:
  ...
  - CLI: `uv run python -m tinylm.train --config smoke [--data-dir D] [--out-dir O] [--steps N] [--device cpu|mps|cuda] [--tokenizer PATH] [--resume] [--no-compile] [--dtype auto|bf16|fp32] [--wandb PROJECT]`.
...
- Consumes: `TinyLM`, `build_optimizers`, `TokenShards`, presets. Data batches drawn with **stateless RNG**: `np.random.default_rng([seed, rank, step, micro])` for train, `np.random.default_rng([seed, 999, k])` for val batch k; this is what makes resume bit-exact without saving loader state.
...
from tinylm.train import lr_scale, run
...
**Files:**
- Create: `tinylm/sample.py`, `tinylm/eval_hellaswag.py`
- Test: `tests/test_eval.py`

**Interfaces:**
- Produces:
  - `sample.py` CLI: `uv run python -m tinylm.sample --ckpt out/smoke/ckpt_last.pt --tokenizer tokenizer/tokenizer.json --prompt "..." [--max-new-tokens 200] [--temperature 0.8] [--top-k 50] [--top-p P] [--device D]`. Loads the checkpoint (`model_cfg` dict → `ModelConfig(**...)`), prints the completion. Exposes `load_model(ckpt_path: str, device: str) -> tuple[TinyLM, ModelConfig]` for reuse.
  - `eval_hellaswag.py` CLI: `uv run python -m tinylm.eval_hellaswag --ckpt PATH --tokenizer PATH [--limit 1000] [--device D]`. Exposes `ending_losses(model, ctx_ids: list[int], ending_ids_list: list[list[int]], device) -> list[tuple[float, float]]` returning `(sum_loss, mean_loss)` per ending; prediction = argmin mean_loss (acc_norm) and argmin sum_loss (acc).
- Consumes: `TinyLM.generate` (Task 6), `BPETokenizer` (Task 2).
...
- [ ] **Step 4: Write `tinylm/eval_hellaswag.py`**
...
Run: `uv run python -m tinylm.train --config smoke --device mps --tokenizer tokenizer/tokenizer.json`
...
Run: `uv run python -m tinylm.train --config smoke --device mps --tokenizer tokenizer/tokenizer.json --steps 620 --resume`
Expected: resumes at step 600, runs 20 more steps.
Run: `uv run python -m tinylm.sample --ckpt out/smoke/ckpt_last.pt --tokenizer tokenizer/tokenizer.json --prompt "The" --max-new-tokens 60`
...
- [ ] **Step 3: Set site identity**: app title "tiny-lm", description "A ~500M-parameter LLM pretrained from scratch"; root page redirects or links to `/docs`.
...
**Interfaces:**
- Consumes: real source code from `tinylm/` (snippets must be copied from the actual files, not paraphrased) and `docs/superpowers/specs/smoke-results.md`.
...
- [ ] **Step 4: Spec conformance sweep**: walk `docs/superpowers/specs/2026-07-17-tiny-lm-500m-design.md` section by section; confirm each requirement is implemented or explicitly documented as cloud-phase (full pretrain, HellaSwag on trained model, dress rehearsal).
...
- Task 8 review hardening (code diverges from the plan's reference blocks, intentionally): sampler weights by `usable+1` valid crop offsets (fixes exact-length-shard off-by-one); `ShardWriter` raises on re-writing an existing split; sampler tests use distinct-content shards + boundary + guard tests.
